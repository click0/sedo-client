...
3. **PIN-ліміт**: спершу підтвердіть PIN одним `--list-objects`, і лише потім
   `--sign`. Лічильник зменшує невдалий `C_Login`, не відхилений `C_Sign`, але
   `pkcs11-tool` на кожен виклик робить власний `--login`, тож із неправильним
   PIN кожна спроба підпису коштує однієї з 15. `opensc-test-almaz.ps1`
   зупиняється на першій невдачі логіну саме тому.
