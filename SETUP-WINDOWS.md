...
**Скільки спроб коштує кожна команда.** `pkcs11-tool` не тримає сесію між
викликами: кожна операція робить власний `--login`, тож при **неправильному**
PIN кожен виклик зменшує лічильник. Лічильник зменшує лише невдалий
`C_Login`, а відхилений `C_Sign` — ні. Скрипт зупиняється на першій невдачі
логіну, тому ціна помилки — рівно одна спроба:

| Команда | Спроб при неправильному PIN |
|---|---|
| `-Pin XXXX` | 1, далі скрипт виходить |
| `-Pin XXXX -TestSign` | 1, до підпису не доходить |
| `-Pin XXXX -TestSign -AllMechanisms` | 1 (а при правильному PIN — до 3 викликів `--sign`, які лічильник не чіпають) |

```powershell
# 5.1 Валідація PIN через запит об'єктів (1 невдача = 1 зі 15 лімітів,
#     після чого скрипт зупиняється сам)
.\opensc-test-almaz.ps1 -Pin XXXX

# Якщо успішно — побачите list-objects з сертифікатом

# 5.2 Тест підпису (ОДИН --sign з mechanism 0x80420031)
.\opensc-test-almaz.ps1 -Pin XXXX -TestSign

# 5.2b Якщо основний механізм не підійшов — дозволити альтернативні
#      (0x80420032 і стандартний 0x00000352; ST-338 «Автор» теж має 0x80420031)
.\opensc-test-almaz.ps1 -Pin XXXX -TestSign -AllMechanisms
...
Перебирати механізми безпечно **лише після** того, як PIN підтверджено через
`--list-objects`: відхилений `C_Sign` лічильник не зменшує, а невдалий
`C_Login` — зменшує, і `pkcs11-tool` робить `--login` на кожен виклик.
